...
function getSearchResultTest(text) {
    ...
    //UAT
    var url = 'https://search-index-uat.search.windows.net/indexes/axisbank-web-search/docs?api-version=2016-09-01&api-key=25B8F4762AA057545E50B03933A6B866&queryType=simple&searchMode=any&scoringProfile=WeightageAndMagnitude&$top=15&search=' + text + '*&$select=Title,URI,Priority,Category,CTAText,CTAURL,IconForSearch,SearchDescription';
    ...
    if (datapresent) {
        ...
        for (i = 0; i < data.length; i += 1) {
            ...
            if (data[i].URI.indexOf('http://') + data[i].URI.indexOf('https:///') == -2) {
                data[i].URI = 'https://www.axisbank.com' + data[i].URI;
            }
...
function BindAzureSearchResults(NavAzureSearchResultscurrentpage, NavAzureSearchResultspagesize) {
    ...
    //UAT
    var url = 'https://search-index-uat.search.windows.net/indexes/axisbank-web-search/docs?api-version=2016-09-01&api-key=25B8F4762AA057545E50B03933A6B866&queryType=simple&searchMode=any&scoringProfile=WeightageAndMagnitude&search=' + searchText + '*&$count=true&$top=' + NavAzureSearchResultspagesize + '&$skip=' + NavAzureSearchResultsskip + '&$filter=Category ne \'banner\'&$select=Id,Title,Description,URI,Priority,ImageURL,Category,CTAText,CTAURL,IconForSearch';

    //Live PR
    //var url = 'https://axisbanklivesearch.search.windows.net/indexes/axisbank-web-search-pr/docs?api-version=2016-09-01&api-key=7BEA5F91C25D2484804D77BC82D60644&queryType=simple&searchMode=any&scoringProfile=WeightageAndMagnitude&search=' + searchText + '*&$count=true&$top=' + NavAzureSearchResultspagesize + '&$skip=' + NavAzureSearchResultsskip + '&$filter=Category ne \'banner\'&$select=Id,Title,Description,URI,Priority,ImageURL,Category,CTAText,CTAURL,IconForSearch';

    //Live DR
    //var url = 'https://axisbanklivesearch.search.windows.net/indexes/axisbank-web-search-dr/docs?api-version=2016-09-01&api-key=7BEA5F91C25D2484804D77BC82D60644&queryType=simple&searchMode=any&scoringProfile=WeightageAndMagnitude&search=' + searchText + '*&$count=true&$top=' + NavAzureSearchResultspagesize + '&$skip=' + NavAzureSearchResultsskip + '&$filter=Category ne \'banner\'&$select=Id,Title,Description,URI,Priority,ImageURL,Category,CTAText,CTAURL,IconForSearch';
